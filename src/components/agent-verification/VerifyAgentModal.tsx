import { CalendarIcon, DocumentIcon, ExclamationTriangleIcon, XMarkIcon } from "@heroicons/react/24/outline";
  ...
  return (
    <div className='fixed flex h-full w-full inset-0 items-center justify-center z-50 bg-black/30 backdrop-blur-sm'>
      <div className='bg-white p-3 sm:p-6 max-w-xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl relative mx-4'>
        ...
        {/* Header */}
        <div className='mb-6'>
          <h2 className='text-lg sm:text-2xl font-bold text-text-primary'>{t("Verify Agent")}</h2>
          <p className='text-sm text-text-secondary mt-1'>
            {t("Review the agent details and set the verification validity period")}
          </p>
        </div>
        ...
        <div className='flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar3'>
          ...
          <div className='shadow-sm border border-inputBorder p-3 sm:p-5'>
            <div className='flex items-start gap-4'>
              ...
              <div className='flex-1'>
                <div className="flex items-center gap-2 mb-3">
                  <span className='text-text-secondary min-w-[90px]'>{t("Name")}:</span>
                  ...
                  <div className='flex items-start gap-2'>
                    ...
                    <div className="flex flex-col">
                      ...
                      {selectedAgent.agentCode?.startsWith('TEMP-') && (
                        <span className="text-xs text-yellow-600 font-medium">{t("Temporary")}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-start gap-2">
            ...
            {(selectedAgent.docLink1 || selectedAgent.docLink2 || selectedAgent.docLink3) && (
              <div className="mt-4 pt-4">
                <p className="text-base font-medium text-text-primary mb-2">{t("Documents")}:</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-purple-900">
                    {t("WFG Agent Verification")}
                  </p>
                  <p className="text-sm text-purple-700 mt-1">
                    <strong>{t("WFG Code")}:</strong> <span className="font-mono bg-white px-2 py-0.5 rounded border border-purple-100">{selectedAgent.wfgCode}</span>
                  </p>
                  <p className="text-xs text-purple-600 mt-2">
                    {t("Please verify this WFG code with WFG before approving this application.")}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col items-start gap-3 mb-4">
                ...
                <div>
                  <p className="font-medium text-yellow-900">
                    {t("Admin Assignment Required")}
                  </p>
                  <p className="text-sm text-yellow-700">
                    {t("This is a public registration. Please assign credentials before verification.")}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-text-primary mb-1">
                    {t("Agent Code")} <span className="text-red-500">*</span>
                  </label>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className={`px-3 transition ${agentCodeAvailability.lastChecked === adminAssignments.agentCode && agentCodeAvailability.status === 'available'
                    ...
                    >
                      {agentCodeAvailability.status === 'checking' ? t('Checking...') :
                        agentCodeAvailability.lastChecked === adminAssignments.agentCode && agentCodeAvailability.status === 'available' ? t('Available') :
                ...
                {!isWfgAgent && (
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-1">
                      {t("Commission Percentage")} <span className="text-red-500">*</span>
                    </label>
                ...
                {needsMgaAssignment && (
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-1">
                      {t("Assign to MGA")} <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={mgaSearch}
                      onChange={(e) => setMgaSearch(e.target.value)}
                      placeholder={t("Search MGA name or code...")}
                      className="input-primary"
                    />

                    {mgasLoading && <p className="text-xs text-text-secondary mt-1">{t("Loading MGAs...")}</p>}
                    ...
                    {mgaSearch.length > 0 && (
                      mgas.length > 0 ? (
                        ...
                        <p className="text-xs text-text-secondary mt-1">{t("No MGAs found matching")} "{mgaSearch}"</p>
                      )
                    )}

                    {adminAssignments.mgaId && (
                      <div className="mt-2 bg-green-50 border border-green-200 px-2 py-1 text-sm text-green-800">
                        <strong>{t("Selected MGA")}:</strong>{' '}
            ...
            <div>
              <div className='flex items-center gap-2 mb-1'>
                ...
                <span className='text-sm font-semibold text-text-primary'>
                  {t("Verification Valid Until")} <span className="text-red-500">*</span>
                </span>
              </div>
              <p className='text-xs text-text-secondary mb-2'>
                {t("Set the date until which this agent's verification will remain valid")}
              </p>
          ...
          <div className='bg-blue-50/50 border border-blue-200 p-3 sm:p-5 mb-2'>
            <p className='text-sm text-blue-800'>
              <span className='font-semibold'>{t("Note")}:</span> {t("Once verified, the agent will have access to all features until the specified validity date. You can always update this later if needed.")}
            </p>
          </div>
        </div>
